...
def list_criterios(db, licitacion_id):
    return db.execute("""
        SELECT c.*, t.TipoCriterio, f.NombreFormula 
        FROM criterios c
...
def create_criterio(db, nombre, descripcion, peso, tipo_id, lic_id, formula_id):
    db.execute("""
        INSERT INTO criterios 
        (NombreCriterio, Descripcion, Peso, tipocriterio_id, licitacion_id, formula_id)
        VALUES(?,?,?,?,?,?)
    """, (nombre, descripcion, peso, tipo_id, lic_id, formula_id))
    db.commit()

def edit_criterio(db, criterio_id, nombre, descripcion, peso, tipo_id, formula_id):
    db.execute("""
        UPDATE criterios
        SET NombreCriterio=?, Descripcion=?, Peso=?, tipocriterio_id=?, formula_id=?
        WHERE id=?
    """, (nombre, descripcion, peso, tipo_id, formula_id, criterio_id))
    db.commit()
